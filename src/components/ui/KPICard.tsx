...
export function KPICard({
  ...
  return (
    ...
    >
      ...
      <CardContent>
        <div
          className={`text-[16px] font-medium tabular-nums ${toneClasses[tone]}`}
        >
          {value}
          {unit ? (
            <span className="ml-1 text-[14px] font-light text-slate-400">
              {unit}
            </span>
          ) : null}
        </div>
        {delta ? (
          <p
            className={`text-[14px] font-light mt-1 flex items-center gap-1 ${trend ? trendColors[trend] : "text-slate-400"}`}
          >
